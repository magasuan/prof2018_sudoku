...
//		}
		...
		int rule_4 = validateRule4(columns);
		if(rule_4 == -4) {
			return -4;
		}
		return 0;
	...
	private static int validateRule3(String[] rows) {
		// TODO Auto-generated method stub
		int res = 0;
		char[] arrayCh;
		int[] arrayNum;
		for (int i = 0; i < rows.length; i++) {
			arrayCh = rows[i].toCharArray();
			arrayNum = new int[9];
	...
	private static int validateRule4(String[] columns) {
		// TODO Auto-generated method stub
		int res = 0;
		char[] arrayCh;
		int[] arrayNum;
			for (int i = 0; i < columns.length; i++) {
				arrayCh = columns[i].toCharArray();
				arrayNum = new int[9];
				for (int j = 0; j < arrayNum.length; j++) {
					arrayNum[j] = Character.getNumericValue(arrayCh[j]);
				}
